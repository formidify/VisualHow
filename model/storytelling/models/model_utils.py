...
class TitleEncoder(nn.Module):
    def __init__(self, opt):
        ...
        self.num_layers = opt.num_layers
        self.hidden_dim = opt.hidden_dim
        self.dropout = opt.title_dropout

        # title embedding layer
        self.title_emb = nn.Embedding(self.vocab_size, self.embed_dim)
    ...
    def forward(self, input, hidden=None):
        """
        inputs:
        - input  	(batch_size, max_title_size)
        - hidden 	(num_layers * num_dirs, batch_size, hidden_dim // 2)
        return:
        - out 		(batch_size, rnn_size), serve as context
        """
        batch_size, seq_length = input.size(0), input.size(1)

        # visual embeded
        emb = self.title_emb(input) # (Na, max_title_size, embedding_size)

        # visual rnn layer
        if hidden is None:
            hidden = self.init_hidden(batch_size, bi=False, dim=self.hidden_dim)
        rnn_input = self.hin_dropout_layer(emb)  # apply dropout
        houts, hidden = self.rnn(rnn_input, hidden)

        out = self.project_layer(houts)
        out = self.relu(out)  # (batch_size, max_title_size, embed_dim)
