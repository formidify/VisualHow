...
import time
import numpy as np
from .model_utils import AttentionLayer, VisualEncoder, TitleEncoder, _smallest
from models.encoders import get_image_encoder
...
class BaseModel(nn.Module):
    def __init__(self, opt):
        ...
        self.opt = opt
        if opt.use_title:
            self.decoder_input_dim = self.word_embed_dim + self.word_embed_dim + self.word_embed_dim
        else:
            self.decoder_input_dim = self.word_embed_dim + self.word_embed_dim
        self.ss_prob = 0.0  # Schedule sampling probability

        self.image_encoder = get_image_encoder(opt.data_name, opt.img_dim, opt.embed_size, opt, opt.precomp_enc_type, opt.backbone_source, opt.backbone_path, opt.no_imgnorm)
        ...
        # Title Encoder
        if opt.use_title:
            self.title_encoder = TitleEncoder(opt)
    ...
    def decode(self, imgs, last_word, state_d, penalize_previous=False, title=None):
        ...
        if title is None:
            ...
            input_d = torch.cat([word_emb, imgs.unsqueeze(1), title.unsqueeze(1)], 2)  # batch_size * 1 * dim
        input_d = self.project_d(input_d)

        out_d, state_d = self.decoder(input_d, state_d)

        log_probs = F.log_softmax(self.logit(out_d[:, 0, :]), dim=-1)
    ...
    def forward(self, features, caption, title):
        """
        :param features: (batch_size, max_length, feat_size)
        :param caption: (batch_size, max_length, seq_length)
        :param title: (batch_size, max_title_length)
        :return:
        """
        self.story_size = features.shape[1]

        # encode the title features
        if self.opt.use_title:
            out_title_e, _ = self.title_encoder(title)

        # reshape the inputs, making the sentence generation separately
        caption = caption.view(-1, caption.size(2))
        if self.opt.use_title:
            title_length = ((title > 0).sum(-1, keepdim=True)).long().unsqueeze(-1).expand(-1, -1, out_title_e.size(2))
            out_title_e = torch.gather(out_title_e, dim=1, index=title_length)
            out_title_e = out_title_e.expand(-1, self.story_size, -1)
            out_title_e = out_title_e.contiguous()
            out_title_e = out_title_e.view(-1, out_title_e.size(2))

        img_mask = (features.sum(-1).sum(-1).sum(-1) != 0).float()
        B, L, C, H, W = features.shape
        reshape_features = features.view(-1, C, H, W)
        features_embedding,  pool_weights = self.image_encoder(reshape_features.view(-1, C, H, W), out_title_e)
        features_embedding = features_embedding.view(B, L, -1)
        # encode the visual features
        out_e, _ = self.encoder(features_embedding)
        ...
        state_d = self.init_hidden_with_feature(features_embedding)
        ...
        for i in range(self.seq_length):
            if self.opt.use_title:
                log_probs, state_d = self.decode(out_e, last_word, state_d, title=out_title_e)
            else:
        ...
        return outputs.view(-1, self.story_size, outputs.size(1), self.vocab_size),  pool_weights

    def sample(self, features, title, sample_max, rl_training=False, pad=False):
        self.story_size = features.shape[1]
        # encode the title features
        if self.opt.use_title:
            out_title_e, _ = self.title_encoder(title)
        ...
        B, L, C, H, W = features.shape
        reshape_features = features.view(-1, C, H, W)
        features_embedding, _ = self.image_encoder(reshape_features.view(-1, C, H, W), out_title_e)
        features_embedding = features_embedding.view(B, L, -1)
        # encode the visual features
        out_e, _ = self.encoder(features_embedding)
        ...
        for t in range(self.seq_length):
            ...
            if self.opt.use_title:
                log_probs, state_d = self.decode(out_e, last_word, state_d, True, title=out_title_e)
            else:
        ...
        if rl_training:
            ...
            seq *= img_mask.data.long().unsqueeze(-1)
            seq_log_probs *= img_mask.unsqueeze(-1)
            baseline *= img_mask.unsqueeze(-1)
    ...
    def topK(self, features, beam_size=5):
        ...
        caption = caption.view(-1, caption.size(2))
        if self.opt.use_title:
            title_length = ((title > 0).sum(-1, keepdim=True)).long().unsqueeze(-1).expand(-1, -1,
                                                                                           out_title_e.size(2))
            out_title_e = torch.gather(out_title_e, dim=1, index=title_length)
            out_title_e = out_title_e.expand(-1, self.story_size, -1)
            out_title_e = out_title_e.contiguous()
    ...
    def predict(self, features, title=None, beam_size=5):
        assert beam_size <= self.vocab_size and beam_size > 0
        if beam_size == 1:  # if beam_size is 1, then do greedy decoding, otherwise use beam search
            return self.sample(features, title, sample_max=True, rl_training=False)
        ...
        for k in range(batch_size):
            ...
            state_d_k = state_d[:, k, :].unsqueeze(1).expand(state_d.size(0), beam_size, state_d.size(2)).contiguous()
            if self.opt.use_title:
                out_title_e_k = out_title_e[k].unsqueeze(0).expand(beam_size, out_e.size(1)).contiguous()

            last_word = Variable(torch.FloatTensor(beam_size).long().zero_().cuda())  # <BOS>
            if self.opt.use_title:
                log_probs, state_d_k = self.decode(out_e_k, last_word, state_d_k, True, title=out_title_e_k)
            else:
                log_probs, state_d_k = self.decode(out_e_k, last_word, state_d_k, True)
            log_probs[:, 1] = log_probs[:, 1] - 1000  # never produce <UNK> token
            neg_log_probs = -log_probs
