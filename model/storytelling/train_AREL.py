...
def train(opt):
    ...
    dataset.set_option(data_type={'whole_story': False, 'split_story': True, 'caption': False, 'title': True})
    ...
    for epoch in range(logger.epoch_start, opt.max_epochs):
        ...
        for iter, batch in enumerate(train_loader):
            ...
            title = Variable(batch['title']).cuda()
            index = batch['index']

            story_size = target.shape[1]
            title_repeat = title.unsqueeze(1).expand(-1, story_size, -1).contiguous()
            ...
            if flag.flag == "Disc":
                ...
                if opt.decoding_method_DISC == 'sample':
                    seq, seq_log_probs, baseline = model.sample(feature_fc, title, sample_max=False, rl_training=True,
                                                                pad=True)
                elif opt.decoding_method_DISC == 'greedy':
                    seq, seq_log_probs, baseline = model.sample(feature_fc, title, sample_max=True, rl_training=True,
                                                                pad=True)
            else:
                model.train()
                disc.eval()
                seq, seq_log_probs, baseline = model.sample(feature_fc, title, sample_max=False, rl_training=True, pad=True)
            ...
            # B, L, C, H, W = feature_fc.shape
            # reshape_features = feature_fc.view(-1, C, H, W)
            # features_embedding = model.image_encoder(reshape_features.view(-1, C, H, W))
            # disc_feature_fc = features_embedding.view(B, L, -1)

            # encode the title features
            if model.opt.use_title:
                out_title_e, _ = model.title_encoder(title)
            ...
            if model.opt.use_title:
                ...
                out_title_e = out_title_e.expand(-1, model.story_size, -1)
                out_title_e = out_title_e.contiguous()
                out_title_e = out_title_e.view(-1, out_title_e.size(2))

            img_mask = (feature_fc.sum(-1).sum(-1).sum(-1) != 0).float()
            B, L, C, H, W = feature_fc.shape
            reshape_features = feature_fc.view(-1, C, H, W)
            features_embedding,  pool_weights = model.image_encoder(reshape_features.view(-1, C, H, W), out_title_e)
            disc_feature_fc = features_embedding.view(B, L, -1)

            # gen_score = disc(seq.view(-1, seq.size(2)), feature_fc.view(-1, feature_fc.size(2)), title_repeat.view(-1, title_repeat.size(2)))
            gen_score = disc(seq.view(-1, seq.size(2)), disc_feature_fc.view(-1, disc_feature_fc.size(2)),
                             title_repeat.view(-1, title_repeat.size(2)))

            if flag.flag == "Disc":
                # gt_score = disc(target.view(-1, target.size(2)), feature_fc.view(-1, feature_fc.size(2)), title_repeat.view(-1, title_repeat.size(2)))
                gt_score = disc(target.view(-1, target.size(2)), disc_feature_fc.view(-1, disc_feature_fc.size(2)),
                                title_repeat.view(-1, title_repeat.size(2)))
            ...
            if flag.flag == "Disc":
                ...
                tf_loss = crit(model(feature_fc, target, title)[0], target)
