...
def decode_story_by_step(id2word, result):
    """
    :param id2word: vocab
    :param result: (batch_size, story_size, seq_length)
    :return:
    out: a list of stories whose element is a list of sentences. the size of the list is batch_size
    """
    batch_size, story_size, seq_length = result.size()
    out = []
    for i in xrange(batch_size):
        ...
        for j in xrange(story_size):
            ...
            for k in xrange(seq_length):
                ...
                if vocab_id > 0:
                    ...
                    break
            if txt != '':
                step_content.append(txt)
        out.append(step_content)
    return out
