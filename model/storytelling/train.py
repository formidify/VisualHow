...
def attention_loss(pred, gt, mask=None):
    ...
    gt = Variable(gt)
    loss = -((gt*torch.log(torch.clamp(pred,min=eps,max=1)))*mask.float()).sum(-1)
...
def train(opt):
    ...
    dataset.val()
    val_loader = DataLoader(dataset, batch_size=opt.batch_size // 2, shuffle=False, num_workers=opt.workers,
                              collate_fn=dataset.collate_func)
    ...
    for epoch in range(logger.epoch_start, opt.max_epochs):
        ...
        for iter, batch in enumerate(train_loader):
            ...
            index = batch['index']
            image_attention = batch['image_attention'].cuda()

            image_mask = feature_fc.sum(-1).sum(-1).sum(-1) != 0
            image_mask = image_mask.view(-1, 1)

            optimizer.zero_grad()

            # cross entropy loss
            output, pool_weights = model(feature_fc, target, title)
            ...
            image_attention = (image_attention + 1e-10) / (image_attention + 1e-10).sum(-1, keepdim=True).sum(-2, keepdim=True).sum(-3, keepdim=True)
            image_attention = image_attention.view(-1, 64)
            pool_weights = pool_weights.view(-1, 64)
            image_mask = image_mask.expand(-1, 64).contiguous()

            att_loss = attention_loss(pool_weights, image_attention, mask=image_mask)

            if opt.aggregate_mode == "att":
                loss = loss + opt.attention_loss_weight * att_loss
...
def test(opt):
    ...
    model = models.setup(opt)
    model.cuda()
    # predictions, metrics = evaluator.test_upper_bound(model, dataset, test_loader, opt)
