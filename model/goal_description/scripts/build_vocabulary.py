...
parser.add_argument(
    "-c",
    "--captions-jsonpath",
    default="data/coco/captions_train2014.json",
    help="Path to COCO train2017 captions json file.",
)
parser.add_argument("-t", "--word-count-threshold", type=int, default=2)
parser.add_argument(
    "-o",
    "--output-dirpath",
...
SPECIAL_TOKENS: List[str] = ["@@UNKNOWN@@", "@@BOUNDARY@@", "@@SEPARATE@@"]
...
if __name__ == "__main__":
    ...
    caption_info = json.load(open(args.captions_jsonpath))
    captions_json = []

    for key, value in caption_info.items():
        if value["split"] == "train":
            captions_json.append({"caption": value["task_title"] + " " + value["method_title"]})
