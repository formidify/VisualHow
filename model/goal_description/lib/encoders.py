...
def get_captioner(opt):
    return Captioner(opt)
...
class EncoderImageFull(nn.Module):
    def __init__(self, backbone_cnn, img_dim, embed_size, precomp_enc_type='basic', aggregate_mode='gpo',
        ...
        # self.image_encoder = EncoderImageAggr(img_dim, embed_size, precomp_enc_type, aggregate_mode, attention_loss,
        #                                       no_imgnorm)
...
class MultiModalEncoder(nn.Module):
    def __init__(self, opt):
        ...
        if self.aggregate_mode == 'att':
            ...
            self.caption_attention_encoder = CaptionAttention(self.embed_size, self.img_dim, self.embed_size // 2, self.attention_loss)
    ...
    def forward(self, image_features, image_lengths, feature_index, cap_emb, cap_len):
        ...
        cap_pool_weights = self.caption_attention_encoder(cap_emb, image_avg_feature, cap_emb_mask)
        # goal_pool_weights = self.goal_attention_encoder(goal_emb, image_avg_feature, cap_cls_feature, goal_emb_mask)
        img_emb, image_pool_weights = self.image_encoder(image_features, cap_cls_feature, image_lengths)
...
class EncoderRNN(nn.Module):
    def __init__(self, opt):
        ...
        self.embed_dim = opt.embed_size * 2
    ...
    def forward(self, input, hidden=None):
        ...
        # visual rnn layer
        if hidden is None:
            hidden = self.init_hidden(input, batch_size, bi=True, dim=self.hidden_dim // 2)
        rnn_input = self.hin_dropout_layer(input)  # apply dropout
        self.rnn.flatten_parameters()
        houts, hidden = self.rnn(rnn_input, hidden)

        # residual layer
        # out = input + self.project_layer(houts)
        out = self.project_layer(houts)
...
class Captioner(nn.Module):
    def __init__(self, opt):
        ...
        self.image_feature_size = opt.embed_size * 2
        self.embedding_size = opt.embedding_size
        self.hidden_size = opt.hidden_size
        self.attention_projection_size = opt.attention_projection_size
        self.beam_size = opt.beam_size
        self.max_caption_length = opt.max_caption_length
        self.use_cbs = opt.use_cbs
        self.min_constraints_to_satisfy = opt.min_constraints_to_satisfy

        self.model = UpDownCaptioner(self.vocabulary,
                                     ...
                                     self.beam_size,
                                     self.use_cbs,
                                     self.min_constraints_to_satisfy)


    def forward(self, image_features, caption_tokens=None):
        return self.model(image_features, caption_tokens)