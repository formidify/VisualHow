...
def select_topk_beam_with_constraints(
    beams: torch.Tensor,
    beam_log_probabilities: torch.Tensor,
    given_constraints: torch.Tensor,
    min_constraints_to_satisfy: int = 2,
    ...
    topk_beams: List[torch.Tensor] = []
    topk_beam_log_probabilities: List[torch.Tensor] = []
    ...
    for i in range(batch_size):
        ...
        valid_beams = beams[i, valid_states, :, :]
        valid_beam_log_probabilities = beam_log_probabilities[i, valid_states, :]

        top_k = valid_beam_log_probabilities.view(-1).argsort(descending=True)[:beam_size]
        topk_valid_beams = valid_beams.view(-1, max_decoding_steps)[top_k]
        topk_valid_beam_log_probabilities = valid_beam_log_probabilities.view(-1)[top_k]

        for index in range(beam_size):
            topk_beams.append(topk_valid_beams[index])
            topk_beam_log_probabilities.append(topk_valid_beam_log_probabilities[index])
        # selected_index = torch.argmax(valid_beam_log_probabilities)
        # best_beams.append(valid_beams[selected_index, :])
        # best_beam_log_probabilities.append(valid_beam_log_probabilities[selected_index])

    # shape: (batch_size, max_decoding_steps)
    return torch.stack(topk_beams).long().to(beams.device), torch.stack(topk_beam_log_probabilities).to(beams.device)
